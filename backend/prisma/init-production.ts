...
async function main() {
  ...
  const explicitPassword = process.env.QUAN_TRI_MAT_KHAU?.trim();
  ...
  if (existingAdmin) {
    ...
    // Đảm bảo trạng thái kích hoạt và gán đầy đủ vai trò
    await prisma.nguoi_dung.update({
      where: { id: existingAdmin.id },
      data: {
        trang_thai: true,
        da_xoa: false,
        ten_dang_nhap: existingAdmin.ten_dang_nhap || adminUsername,
      },
    });
  ...
  // 5. Bảo đảm các danh mục chuẩn tồn tại (không làm ảnh hưởng dữ liệu đang có)
  const chucVuSample = [
