            ...
            <div className="space-y-8">
              ...
              <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-8 space-y-4 shadow-xl transition-colors">
                ...
                <div className="pt-4 border-t border-slate-200 dark:border-slate-800 flex flex-wrap items-center gap-6 text-xs text-slate-600 dark:text-slate-400">
                  <div>👥 Tổng số thành viên: <strong className="text-[#E97036] font-bold">{danhSachGiaoVien.length} Giáo viên</strong></div>
                ...
                {danhSachGiaoVien.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                    {danhSachGiaoVien.map((gv) => {
                      const chucVuDisplay = gv.danh_muc_chuc_vu?.ten || gv.chuc_vu || 'Giáo viên';
                      const boMonDisplay = gv.danh_muc_bo_mon?.ten;
                      const roleDisplay = [chucVuDisplay, boMonDisplay].filter(Boolean).join(' - ');

                      return (
                        <Link
                          key={gv.id}
                          href={`/giao-vien/${gv.id}`}
                          className="group bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-[#E97036] rounded-2xl p-6 transition shadow-xl text-center space-y-3 flex flex-col items-center justify-between cursor-pointer"
                        >
                          <div className="space-y-3 w-full">
                            <div className="w-16 h-16 rounded-full bg-slate-100 dark:bg-slate-950 border-2 border-[#E97036]/30 overflow-hidden mx-auto flex items-center justify-center font-bold text-[#E97036] text-xl shadow-md">
                              {gv.anh_dai_dien ? (
                            ...
                            <div>
                              <h3 className="font-extrabold text-slate-900 dark:text-white text-sm group-hover:text-[#E97036] transition">{gv.ho_ten}</h3>
                              <p className="text-xs font-semibold text-[#E97036] mt-0.5">{roleDisplay}</p>
                            </div>
                          </div>
